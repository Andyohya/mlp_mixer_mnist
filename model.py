...
class MlpMixer(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, train: bool):
        s = self.patch_size                 # 就是kernel_size
        # print("stem conv 輸入 x 陣列 shape:", x.shape)
        x = nn.Conv(self.hidden_dim, (s, s), strides=(s, s), name='stem')(x)
        # np.set_printoptions(threshold=np.inf, linewidth=200)  # 顯示所有元素
        # print("stem conv 輸出 x 陣列 shape:", x.shape)
        # print(np.array(x))
